...
static uint32_t mount_partition(struct list_elem * pelem, int arg){
...
int32_t sys_chdir(const char * path){
	...
	if(inode_no != -1){
		if(searched_record.file_type == FT_DIRECTORY){
			...
			printk("sys_chair: %s is regular file or other!\n", path);
		}
	}
	dir_close(searched_record.parent_dir);
	return ret;
...
void sys_help(void){
    ...
    clear: clear screen\n\
    help: show all buildin commands\n\
shortcut key:\n\
    ctrl+l: clear screen\n\
    ctrl+u: clear input\n\n");
}
