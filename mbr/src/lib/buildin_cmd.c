		...
		if(long_info){
			...
			while((dir_e = readdir(dir))){
				...
				if(stat(sub_pathname, &file_stat) == -1){
					printf("ls: cannot access %s: No such file directory\n", dir_e->filename);
					return;
				}
				printf("%c  %d  %d  %s\n", f_type, dir_e->i_no, file_stat.st_size, dir_e->filename);
			}
		}else{
...
int32_t buildin_touch(uint32_t argc, char ** argv){
	...
	if(argc != 2){
		...
		make_clear_abs_path(argv[1], final_path);
		if(strcmp("/", final_path)){
			uint32_t fd = open(final_path, O_CREAT|O_RDWR);
			if(fd != -1){
				close(fd);
				ret = 0;
			}else{
				printf("touch: create %s failed.\n", argv[1]);
			}
		}
	}
	return ret;
